...
def load_data(ticker1, ticker2, start, end):
    df1 = yf.download(ticker1, ticker2, end)
    df2 = yf.download(ticker1, ticker2, end)
    return df1, df2
...
def plot_data(data, indicators, sync_axis = None):
    ...
    p.vbar(df.index[loss], width, df.Open[loss], df.Close[loss], fill_color = "#ff0000", line_color = "ff00f00")
...
date_picker_to = DatePicker(title = "End Date", value = "2020-01-01", min_date = "2000-01-01",
                                max_date = dt.datetime.now().strftime("%Y-%m-%d"))
